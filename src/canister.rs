...
pub fn backend() -> Principal {
    let mut canister_id = dotenv!("CANISTER_ID_ANTHOL_BACKEND").to_string();
    if canister_id.is_empty() {
        canister_id = env::var("CANISTER_ID_ANTHOL_BACKEND")
            .expect("CANISTER_ID_ANTHOL_BACKEND is must be set");
    }
